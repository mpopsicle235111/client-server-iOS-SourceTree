...
final class FriendsViewController: UITableViewController {
    ...
    //friends replaced with cellData to use with for SimpleFactory pattern
    private var cellData: Results<FriendDAO>?
    private var friendsToken: NotificationToken?
    
    //This is for SimpleFactory pattern optimization
    private let viewModelFactory = RegularCellViewModelFactory()
    private var viewModels: [RegularCellViewModel] = []
    ...
    override func viewDidLoad() {
        ...
        friendsAPI.getFriends{ [weak self] cellData in
            ...
            self.cellData = self.friendsDB.fetch()
            ...
            self.friendsToken = self.cellData?.observe(on: .main, { [weak self] changes in
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        guard let viewModels = cellData else { return 0 }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //This is for  SimpleFactory pattern
        func configure(with viewModel: RegularCellViewModel) {
            cell.textLabel?.text = viewModel.regularCellText
            cell.textLabel?.shadowColor = viewModel.regularCellTextShadowColor
            cell.contentView.backgroundColor = viewModel.regularCellBackgroundColor
        }
        
        //This unpacking is for Realm (where friends are optional)
        guard let friends = cellData else { return cell }
        
        let friend = friends[indexPath.row]
        
        //configure(with: viewModels[indexPath.row])
        
        //cell.textLabel?.text = friends[indexPath.row].name
        cell.textLabel?.text = "\(friend.firstName ?? "") \(friend.lastName ?? "")"
        
        //Added to try Flyweight pattern
        cell.textLabel?.shadowColor = Colors.Red.brandRedGrey
